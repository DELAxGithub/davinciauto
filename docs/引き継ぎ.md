...
- `docs/EP039_Platto_DaVinci_Handoff.md` — プラっとEP039のGoogle Doc/CSV/DaVinci Resolve自動カット作業。35分まで短くなった自動カット版から、v3台本に沿って約45分へ復活させるためのコールドスタートメモ。
- `docs/EP039_Doc_to_DaVinci_Recalculated_NA_Runbook.md` — EP039で成功したNA込み再計算タイムラインの再実行手順。`KEEP尺 + NA尺` を足し算してrecord TCを作り、A1/A2/A3へ再配置する本命パイプライン。
- `docs/Doc_to_DaVinci_Automation_Design.md` — EP40以降のDoc/CSV→DaVinci自動反映設計。EP39形式の表を正本にし、Doc/CSV→IR→尺計算→ブロック荒編→新規タイムライン生成へ進めるV0方針。EP40では `040_自動編集_ブロックNA_0628` を生成済み。
- `docs/Platto_Google_Doc_Authoring_Guide.md` — プラっとGoogle Docの書き方マニュアル。編集指示タグ、NA表、優先度、自動化レベル、備考の分け方を定義。
- `docs/Palmier_MCP_Capability_Audit.md` — Palmier MCP機能の棚卸し。Doc→DaVinci自動反映へ採用する設計規律、V1検証対象、捨てる生成AI機能を分類。
- `docs/Platto_Split_Audio_Whisper_Workflow.md` — プラっと収録でWAVが途中分割された場合のWhisper→Google Sheets投入手順。EP040の `040_whisper_0624` 実績と分割オフセット計算ルールを記録。
